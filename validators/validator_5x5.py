...
class RushHour5x5Validator:
    def __init__(self):
        self.max_row, self.max_col = 5, 5
    
    def parse_move(self, move_str: str) -> Optional[Tuple[str, List[List[int]], List[List[int]]]]:
        """Parse a move string for 5x5 puzzles (2x1 and 3x1 pieces)."""
        ...
        # Try 3x1 pattern first
        match_3x1 = re.search(pattern_3x1, move_str, re.IGNORECASE)
        if match_3x1:
            piece, sr1, sc1, sr2, sc2, sr3, sc3, er1, ec1, er2, ec2, er3, ec3 = match_3x1.groups()
            start_pos = [[int(sr1), int(sc1)], [int(sr2), int(sc2)], [int(sr3), int(sc3)]]
            end_pos = [[int(er1), int(ec1)], [int(er2), int(ec2)], [int(er3), int(ec3)]]
    ...
    def is_valid_position(self, pos: List[List[int]]) -> bool:
        """Check if all positions are within 5x5 grid bounds."""
        for p in pos:
            if p[0] < 1 or p[0] > self.max_row or p[1] < 1 or p[1] > self.max_col:
                return False
        return True
    
    def is_adjacent_move(self, start_pos: List[List[int]], end_pos: List[List[int]]) -> bool:
        """Check if move is exactly one square in cardinal direction for all cells."""
        if len(start_pos) != len(end_pos):
            return False
        
        # Calculate movement vector for first cell
        move_vector = [end_pos[0][0] - start_pos[0][0], end_pos[0][1] - start_pos[0][1]]
        
        # Check if it's exactly one square in cardinal direction
        if abs(move_vector[0]) + abs(move_vector[1]) != 1:
            return False
        
        # All cells must move in the same direction
        for i in range(1, len(start_pos)):
            cell_move = [end_pos[i][0] - start_pos[i][0], end_pos[i][1] - start_pos[i][1]]
            if cell_move != move_vector:
                return False
        
        return True
    
    def apply_move(self, pieces_state: Dict[str, Any], piece_name: str, 
                   start_pos: List[List[int]], end_pos: List[List[int]]) -> Tuple[bool, str]:
        """Apply and validate a move for 5x5 puzzles."""
        ...
        if current_sorted != start_sorted:
            return False, f"Start position mismatch for {piece_name}"
        
        if not self.is_valid_position(end_pos):
            return False, f"End position out of bounds for {piece_name}"
        
        if not self.is_adjacent_move(start_pos, end_pos):
            return False, f"Move is not adjacent for {piece_name}"
        ...
        for pos in end_pos:
            if tuple(pos) in occupied_cells:
                return False, f"End position {pos} is occupied"
    ...
    def validate_solution(self, initial_state: Dict[str, Any], 
                         moves: List[str], target_zone: List[List[int]]) -> Dict[str, Any]:
        """Validate complete solution for 5x5 puzzles."""
        ...
        # Check if car reached target zone
        if validation_result['moves_are_legal'] and 'C' in current_state:
            final_car_pos = current_state['C']['positions']
            validation_result['final_car_position'] = final_car_pos
            
            # For 5x5, car must exactly cover the target zone
            final_sorted = sorted([tuple(p) for p in final_car_pos])
            target_sorted = sorted([tuple(p) for p in target_zone])
            validation_result['reaches_target'] = (final_sorted == target_sorted)
        
        return validation_result


class ModelPerformanceAnalyzer5x5:
    def __init__(self, data_path: str, results_path: str):
        self.data_path = data_path
        self.results_path = results_path
        self.validator = RushHour5x5Validator()
    ...
    def load_model_result(self, model_name: str, puzzle_num: int) -> Dict[str, Any]:
        result_folder = os.path.join(self.results_path, model_name, "results5x5", f"puzzle{puzzle_num}")
    ...
    def analyze_model_performance(self, model_name: str, max_puzzles: int = 150) -> Dict[str, Any]:
        ...
        for puzzle_num in range(1, max_puzzles + 1):
            try:
                ...
                target_zone = puzzle_data['target_zone']
                ...
                validation = self.validator.validate_solution(puzzle_data, parsed_moves, target_zone)
    ...
    def analyze_all_models(self, model_names: List[str], max_puzzles: int = 150) -> Dict[str, Any]:
        all_results = {'grid_size': '5x5', 'models': {}}
        
        for model_name in model_names:
            print(f"\n=== Analyzing {model_name} (5x5) ===")
            try:
...
def main():
    DATA_PATH = "/home/mustafaah/rushhoureval/data/5x5"
    ...
    analyzer = ModelPerformanceAnalyzer5x5(DATA_PATH, RESULTS_PATH)
    all_results = analyzer.analyze_all_models(MODEL_NAMES, MAX_PUZZLES)
    analyzer.save_results(all_results, "5x5_validation_results.json")
    
    print("\n" + "="*60)
    print("5x5 BENCHMARK SUMMARY")
    print("="*60)
