...
class Qwen3_8B_4x4RushHourInference:
    def __init__(self, model_name="Qwen/Qwen3-8B", device="auto"):
        """
        Initialize Qwen3-8B model for 4x4 Rush Hour puzzle inference.
        ...
        # System prompt for 4x4 Rush Hour puzzles (copied from 4x4 script)
...
Step 7: C [3,4] -> [2,4]
    ...
    def create_chat_prompt(self, puzzle_prompt: str) -> str:
        """
        Create a chat-formatted prompt for Qwen3-8B model with thinking enabled.
        ...
        # Use tokenizer's chat template if available with thinking enabled
    ...
    def generate_response(self, prompt: str, max_new_tokens: int = 32768, temperature: float = 0.6) -> str:
        """
        Generate response from Qwen3-8B model (using same params as 3x3 script).
    ...
    def save_puzzle_result(self, output_path: str, puzzle_num: int, 
        ...
        result_file = os.path.join(puzzle_result_folder, f"qwen3_8b_4x4_puzzle{puzzle_num}_result.json")
    ...
    def run_inference_on_dataset(self, dataset_path: str = "/root/rushhoureval/data/4x4", 
        ...
        print(f"Starting 4x4 inference with Qwen3-8B model...")
        ...
        print(f"Individual results saved in: {output_path}/puzzle[N]/qwen3_8b_4x4_puzzle[N]_result.json")
    ...
    def save_results_summary(self, results: List[Dict], output_path: str, timestamp: str):
        ...
        json_file = os.path.join(output_path, f"qwen3_8b_4x4_inference_results_{timestamp}.json")
        with open(json_file, 'w', encoding='utf-8') as f:
            json.dump(results, f, indent=2, ensure_ascii=False)
        
        # Save CSV summary
        csv_file = os.path.join(output_path, f"qwen3_8b_4x4_inference_summary_{timestamp}.csv")
...
def main():
    ...
    print("🚀 Starting Qwen3-8B 4x4 Rush Hour Inference")
    print("=" * 60)
    
    # Initialize inference system
    inference = Qwen3_8B_4x4RushHourInference(
